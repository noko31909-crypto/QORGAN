...
import { ScrollView, StyleSheet, Text, View, Pressable, RefreshControl, TextInput, Linking } from 'react-native';
      ...
      >
        ...
        {visibleIncidents.map((inc) => (
          <View key={inc.id} style={styles.incidentCard}>
            ...
            <Text style={styles.item}>Reported: {timeAgo(inc.created_at)}</Text>
            {inc.confidence != null && (
              <Text style={[styles.item, { fontWeight: '700', color: inc.confidence > 0.8 ? '#B91C1C' : '#D97706' }]}>AI confidence: {Math.round(inc.confidence * 100)}%</Text>
            )}
        ...
        <View style={styles.actions}>
          <Pressable style={styles.actionBtn} onPress={() => Linking.openURL('tel:911')}><Text style={styles.actionText}>Ambulance: 911</Text></Pressable>
          <Pressable style={styles.actionBtn} onPress={() => Linking.openURL('tel:100')}><Text style={styles.actionText}>Police: 100</Text></Pressable>
        </View>
      </ScrollView>
    </ScreenShell>
  );
