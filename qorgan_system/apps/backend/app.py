...
if ENABLE_DETECTION:
    ...
    detection_service = DetectionService(
        ...
        persistence_frames=int(os.getenv('PERSISTENCE_FRAMES', '3')),
        ...
        max_bbox_area_ratio=float(os.getenv('MAX_BBOX_AREA_RATIO', '0.7')),
    )
...
def seed_demo_data():
    ...
    seed_rows = [
        Incident(type='weapon_detected', school_code='SCH-1234', description='Demo: knife-like object in hallway', location='Main Hall', confidence=0.86, status='acknowledged', created_at=now - timedelta(days=2), reported_by=guard.id),
        Incident(type='sos_alert', school_code='SCH-1234', description='Demo: SOS test from mobile app', location='Classroom 8', status='resolved', created_at=now - timedelta(days=1), reported_by=guard.id),
        Incident(type='weapon_detected', school_code='SCH-1234', description='Demo: possible threat near entrance', location='Main Entrance', confidence=0.91, status='new', created_at=now - timedelta(hours=3), reported_by=guard.id),
    ]
